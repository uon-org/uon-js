...
export class ListenerWS extends Listener {
    constructor(opt)
    {
        ...
        this.auth        = opt.auth || [];
        this.preAuths    = new Map();

        const serverOptions   = {...opt.serverOptions};
        serverOptions.ca    ||= [];

        for(const a of this.auth)
            if(a.wsSetServerOptions) a.wsSetServerOptions(this, serverOptions);
        ...
        this.wss.on('connection', (ws, req) => {
            let preAuth = this.preAuths.get(req);
            if(preAuth) this.preAuths.delete(req);
    ...
    preAuthorizedBy(req, id) { this.preAuths.set(req, id); }
}
